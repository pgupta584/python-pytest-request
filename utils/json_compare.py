...
def filter_values(dictionary: Dict, ignore_types: List[type] = [], ignore_regex: Optional[dict] = None) -> Dict[
    str, type]:
    """
...
def dict_diff(actual: Dict, expected: Dict, ignore_types: List[type] = [], ignore_regex: Optional[dict] = None) -> Dict[
    str, type]:
    """
...
def schema_extract_types(dictionary: Dict, ignore_types: List[type] = [], ignore_regex: Optional[str] = None) -> Dict[
    str, type]:
    """
    Extract types from (eventually nested) dictionary.
...
def schema_dict_diff(actual: Dict, expected: Dict, ignore_types: List[type] = [], ignore_regex: Optional[str] = None) -> \
        Dict[str, type]:
    """
    Compute differences in two dictionary composition types.
    ...
    def schema_get_diff(actual_types: Dict, expected_types: Dict) -> Dict[str, type]:
        ...
        for k, v in actual_types.items():
            if k not in expected_types:  # Key found in actual but not in expected
                ...
                if isinstance(v, dict):
                    if isinstance(expected_types[k], dict):
                        d = schema_get_diff(v, expected_types[
                            k])  # Just 2 different dataframes, recursively check for differences
                        if d:
                ...
                d = schema_get_diff(v,
                                    expected_types[k])  # Just 2 different dataframes, recursively check for differences
                if d:
                    diffs[k] = d  # Found diffs in sub-dictionaries
